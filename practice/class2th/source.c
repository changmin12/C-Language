...
#pragma region 상수
	...
	// 상수는 메모리 공간을 생성하는 동시에 초기하해야 하며,
	// 한 번 저장된 값은 더 이상 변경할 수 없습니다

	// ex) data = 999;

	// 상수의 경우 메모리 공간을 가지고 있지 않은 상수를
	// 리터널 상수라고 하며, 메모리 공간을 가지고 있는 상수를
	// 심볼릭 상수라고 합니다.
	...
	// 정수형 리터널 상수는 저장하는 값의 범위가 크지 않으면
	// int 자료형으로 저장되며, 실수형 리터널 상수는 double
	// 자료형으로 저장됩니다.
...
#pragma region 비트 연산자
	...
	// 첫 번째 비트는 부호를 나타내며, 첫 번쨰 비트에 1이 있다면 값은
	// 음수가 됩니다.
